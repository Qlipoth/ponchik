Vue.component('fns-service', Vue.extend({
    template: [
        '<div class="col-md-9 serv">',
            '<div class="panel panel-default sg-panel ct">',
                '<div class="scroll-box">',
                    '<div class="panel-heading">',
                        '<h2 class="panel-title" v-if="service">',
                            '<strong>{{service.title}}</strong>',
                        '</h2>',
                    '</div>',
                    '<div class="panel-body service-table">',
                        '<h4><strong>Время отклика</strong></h4>',
                        '<div class="gradtimeline-box">',
                            '<fns-gradtimeline gid="service"></fns-gradtimeline>',
                        '</div>',
                         '<br>',
    ...
    methods: {
        getData: function() {
            ...
            $.post('/getServiceData', { serviceName: decodeURIComponent(location.pathname.split('/').pop())})
            .done(function(response) {
                console.log(response)
                vm.service = response.service;
                // Надеюсь длительность агрегат сам отдаст в нужном формате
                vm.rows = _.map(vm.service.incidents, function(row) {
