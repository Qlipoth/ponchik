var nvd3Charts = function() {
    ...
    var startChart6 = function() {
        d3.json("js/assets/jsplugins/nvd3/linePlusBarData.json", function(error, data) {
            nv.addGraph(function() {
    ...
    window.startChartSingle = function() {
        d3.json('js/assets/plugins/nvd3/cumulativeLineData1.json', function(data) {
            console.log('я вызывалась')
            nv.addGraph(function() {
                ...
                d3.select('#chart-single svg').datum(data).call(chart);
    ...
    window.startChart7 = function() {
        d3.json('js/assets/plugins/nvd3/cumulativeLineData.json', function(data) {
            nv.addGraph(function() {
                var chart = nv.models.cumulativeLineChart().x(function(d) {
                        return d[0];
                    }).y(function(d) {
    ...
    // Изменения состояния
    window.changedStates = function() {
        var labels = ['Устаревшие данные', 'Останов', 'Норма', 'Ошибка', 'Предупреждение', 'Критическая ошибка', ]
        var test_data = stream_layers(6, 12, 0).map(function(data, i) {
            return {
                key: labels[i],
                nonStackable: (i == 1),
                values: data.map(function(a, indx) {
                    var x;
                    if (indx < 10) {
                        x = '2016-0' + (indx || 1) + '-15';
                    } else {
                        x = '2016-' + (indx > 12 ? 12 : indx) + '-' + (indx > 12 ? indx : 15);
                    }
        ...
        nv.addGraph({
            generate: function() {
                ...
                var chart = nv.models.multiBarChart()
                    .width($(this).closest('.panel-body').width())
                    .height(300)
                    .stacked(true);
                chart.xAxis.tickFormat(function(d) {
                    return d3.time.format("%d.%m.%y %H:%M")(moment(d).toDate());
                });
                chart.yAxis.tickFormat(d3.format("d"));
                // chart.dispatch.on('renderEnd', function() {
                //     console.log('Render Complete');
                // });
                var svg = d3.select('#chart1 svg').datum(test_data);
                console.log('calling chart');
                svg.transition().duration(0).call(chart).style({height: 300});
                return chart;
            },
    ...
    window.responseTime = function() {
        var xLabels
        window.testdata = stream_layers(2, 20, .1).map(function(data, i) {
            return {
                key: 'Stream' + i,
                values: data.map(function(a, indx) {
                    var x;
        ...
        nv.addGraph(function() {
            ...
            d3.select('#chart2 svg')
                .datum(testdata)
                .transition().duration(500).call(chart).style({ height: '300px' });
            return chart;
        });
    }
    window.eventCount = function() {

        var testdata = [
            { key: "Критическая ошибка", y: 4, color: "red" },
            // { key: "Two", y: 2 },
            // { key: "Three", y: 9 },
            // { key: "Four", y: 7 },
            // { key: "Five", y: 4 },
        ...
        var height = $('#chart3').closest('.panel').height();
     ...
     window.eventDuration = function() {

        var testdata = [
            { key: "Критическая ошибка", value: 1.38, color: "red" },
            { key: "Нет данных", value: 3.96, color: "blue" },
            { key: "Норма", value: 94.66, color: "green" },
        ];
        var height = $('#chart4').closest('.panel').height();
